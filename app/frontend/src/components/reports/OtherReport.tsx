...
interface OtherReportProps extends ReportBaseProps {
  reportType?: 'other';
}
...
const OtherReport: React.FC<OtherReportProps> = ({
  ...
  const documentTypes = [
    ...
    { value: 'correspondence', label: 'Medical Correspondence' },
    { value: 'education', label: 'Patient Education Material' },
  ...
  const handleUpload = async () => {
    ...
    if (!description) {
      setUploadError('Please provide a description for the document');
      return;
    }
    ...
    try {
      ...
      // Create enhanced description with document type and date
      const enhancedDescription = `${documentTypes.find(d => d.value === documentType)?.label || 'Document'} - ${documentDate} - ${description}`;
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                placeholder="e.g., Surgical consent form, Insurance approval letter"
                value={description}
            ...
            <div className="pt-4">
              <button
                ...
                disabled={isUploading || !selectedFile || !description || !documentType || !reportDate}
                className={`w-full inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white ${isUploading || !selectedFile || !description || !documentType || !reportDate
            ...
            >
              {selectedFile ? (
                ...
                <div className="space-y-1 text-center">
                  ...
                  <p className="text-xs text-gray-500">PDF, Word, Excel, Images up to 10MB</p>
                </div>
              )}
            </div>
          </div>
